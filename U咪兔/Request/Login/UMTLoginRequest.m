...
+ (void)getUserTokenWithUserPhone:(NSString *)userPhone andPassword:(NSString *)password andCompletionBlock:(void(^)(NSError *erro, id response))completionBlock{
    NSDictionary *params = @{@"phone":userPhone,
                             @"password":password};
    [[UMTBaseRequest sharedRequest] requestWithType:UMTRequestTypePost params:params andUrlPath:@"user/login" completionBlock:^(id response, NSString *message, NSError *erro) {
        if (erro) {
            if (completionBlock) {
